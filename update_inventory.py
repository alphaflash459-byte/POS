with open("src/components/InventorySection.tsx", "w") as f:
    f.write("""/**
 * @license
  ...
  return (
    <section id="sec-inventory" className="flex flex-col gap-4 md:gap-6 w-full font-khmer">
      ...
      <div className="bg-white p-5 md:p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
        ...
        <div className="z-10">
          <h2 className="text-base md:text-lg font-black text-slate-800">បញ្ជីទំនិញក្នុងស្តុក</h2>
        </div>

        <div className="flex flex-row gap-2 md:gap-3 w-full md:w-auto items-center z-10">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="overflow-x-auto custom-scroll">
          <table className="w-full text-left text-sm text-slate-600 whitespace-nowrap">
            <thead className="bg-slate-50 text-slate-500 text-[11px] uppercase font-black tracking-wider border-b border-slate-200 font-sans">
              <tr>
                <th className="px-4 py-4 md:px-5">ឈ្មោះទំនិញ</th>
                <th className="px-4 py-4 md:px-5 text-center">កូដសម្គាល់</th>
                <th className="px-4 py-4 md:px-5 text-right">តម្លៃ</th>
                <th className="px-4 py-4 md:px-5 text-center">បរិមាណ</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 font-medium">
              {filtered.length > 0 ? (
                filtered.map((p) => (
                  ...
                  >
                    <td className="px-4 py-3 md:px-5 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl overflow-hidden border border-slate-200 bg-white flex-shrink-0 flex items-center justify-center relative shadow-sm">
                        {p.image ? (
                          <img
                            src={p.image}
                            alt={p.name}
                    ...
                    <td className="px-4 py-3 md:px-5 text-center font-bold font-mono tracking-wider text-slate-500 text-xs">
                      {p.sku || '-'}
                    </td>
                    <td className="px-4 py-3 md:px-5 text-right font-black text-blue-600 text-sm font-sans">
                      ${p.price.toFixed(2)}
                    </td>
                    <td className="px-4 py-3 md:px-5 text-center">
                      <span
                        className={`inline-block px-3 py-1 rounded-lg text-[11px] font-black font-sans ${
                          p.stock <= 5
